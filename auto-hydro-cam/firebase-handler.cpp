...
void FirebaseModule::sendFilesToStorage(const char* _path, uint32_t __time) {
        if (Firebase.ready() && millis() - storageTime >= __time) {
                if (Firebase.Storage.upload(fbdo,
                                            STORAGE_BUCKET_ID /* Firebase Storage bucket id */,
                                            _path /* path to local file */,
                                            mem_storage_type_flash /* memory storage type, mem_storage_type_flash and mem_storage_type_sd */,
                                            _path /* path of remote file stored in the bucket */,
